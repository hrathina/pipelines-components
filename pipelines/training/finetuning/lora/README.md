...
1) Dataset Download - Prepares training data from HuggingFace, S3, or HTTP 2) OCI Model Resolution - Resolves OCI model URIs or passes through HF model IDs 3) LoRA Training - Fine-tunes using unsloth backend (low-rank adapters) 4) Evaluation - Evaluates with lm-eval harness (MMLU, GSM8K, etc.) 5)
